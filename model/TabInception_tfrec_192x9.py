...
class TabCNN:
    ...
    def __init__(self, 
        ...
        self.save_folder = self.save_path + self.spec_repr + "TabInception_tfrec_192x9_" + datetime.datetime.now().strftime("%Y-%m-%d %H%M%S") + "/"
...
# DEFINE TRANSITION BLOCK
        ...
        x = Conv2D(32, 3, padding='same')(model_input)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = MaxPooling2D(pool_size=(4, 1))(x)
        
        x = dense_block(1, 32)(x)
        x = transition_block(32)(x)
        ...
        x = Dropout(0.5)(x)
        #######################################
        x = Dense(self.num_classes * self.num_strings)(x) # no activation
        x = Reshape((self.num_strings, self.num_classes))(x)
        model_output = Activation(self.softmax_by_string)(x)
